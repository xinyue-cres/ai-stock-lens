...
- bias_checks: array，3-5 条**当前绝对不能做的事**，每条形如：
    {
      "bias": "anchoring" | "endowment" | "disposition" | "confirmation"
             | "recency" | "availability" | "loss_aversion" | "overconfidence"
             | "herding" | "sunk_cost",
      "label": "简短标签，如 '破位不走'、'追高加仓'、'扛单幻想'",
      "do_not": "禁止事项。必须以'不要'开头，直接、具体、含当前走势数据。
                 重点关注：不遵守止损纪律、破位后侥幸扛单、追高被套后不走、
                 冲动加仓摊薄等实操错误。
                 例如：'不要在跌破 15.20 止损位后继续持有——MA20 已拐头向下，
                 破位就走是铁律'（<=60 字）",
      "do_instead": "替代行动。必须以'应该'开头，给出具体的纪律动作。
                     例如：'应该在 15.20 设好条件单，触发即卖不犹豫，
                     明天再评估是否回补'（<=50 字）"
    }
  **风格约束**：
    - 这不是学术科普，是盘前纪律提醒——像教练在开盘前喊的"今天不许做X"
    - do_not 必须以"不要"开头，针对的是今天/当前可能犯的具体操作错误
    - do_instead 必须以"应该"开头，给出可执行的纪律动作（含价位/时间点）
    - 每条必须引用当前输入中的具体数据（价格/止损位/MA/支撑阻力）
    - 优先覆盖：止损纪律 > 追高风险 > 加仓冲动 > 心理偏误
    - 如果用户昨天抄底今天面临破位风险，第一条必须是"不要扛单"
    - 结合当前持仓/浮盈浮亏/技术位/冲突程度针对性输出
...
- bias_checks 是纪律禁令，不是学术科普。必须以"不要"+"应该"句式输出，
  直接引用当前价位/止损位/技术位。优先提醒止损纪律和破位风险。"""
