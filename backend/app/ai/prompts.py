...
2. 一份针对当前持仓/仓位状态的行为偏误检查清单（bias_checks），3-5 条散户
   最容易在此情形下犯的心理错误及自查提醒。
...
- bias_checks: array，3-5 条**直接的行为禁令**，每条形如：
    {
      "bias": "anchoring" | "endowment" | "disposition" | "confirmation"
             | "recency" | "availability" | "loss_aversion" | "overconfidence"
             | "herding" | "sunk_cost",
      "label": "偏误中文名，如 '锚定效应'、'近因效应'",
      "do_not": "禁止事项。直接、具体、含当前数据，格式='不要 XXX'。
                 例如：'不要因为今天涨了 3.69% 就追高加仓——日线 MACD 已转负，
                 短期均线发散不代表趋势确立'（<=60 字）",
      "do_instead": "替代行动。直接可执行的正确做法，格式='应该 XXX'。
                     例如：'应该等回踩 MA10(17.85) 再考虑加仓，或直接观望至
                     周线 MACD 翻正'（<=50 字）"
    }
  **风格约束**：
    - do_not 必须以"不要"开头，语气像教练在耳边喊的一句话，不是课本定义
    - do_instead 必须以"应该"开头，给出替代动作而非空洞反问
    - 每条必须引用当前输入中的具体数据（价格/涨跌幅/MA/MACD/换手率等）
    - 禁止输出"如果让你重新买你还会买吗"这种学术反问，直接告诉用户做什么、不做什么
    - 结合当前持仓/浮盈浮亏/冲突程度/当日走势针对性挑选最相关的 3-5 条
...
- bias_checks 必须以"不要"+"应该"的禁令句式输出，直接引用当前数据，
  禁止空洞反问或学术科普"""
