...
- bias_checks: array，**恰好 3 条**当前纪律命令，每条形如：
    {
      "bias": "anchoring" | "endowment" | "disposition" | "confirmation"
             ...
             | "herding" | "sunk_cost",
      "label": "3-6 字动词短语标签，如 '破位必走'、'确认再追'、'挂好止损'",
      "command": "一句话纪律命令，<=30 字。格式为条件式：
                  '禁止在[未满足确认条件]时做[某事]' 或 '必须在[某条件]时做[某事]'。
                  例如：'禁止在量比<1.5时追入突破'
                  例如：'必须在跌破 15.20 时无条件离场'
                  例如：'禁止在未站稳MA20前加仓'"
    }
  **风格约束**：
    - 每条只有一句话，不解释原因
    - 纪律命令必须是**条件式**——"在[条件]下禁止/必须做[动作]"
    - 条件必须是可观测、可验证的（价位、量比、均线关系），不是方向预测
    - 好的纪律 = 事后无论涨跌都不会显得错：
      × 错误："禁止追高到 X 以上"（涨了就打脸）
      ✓ 正确："禁止在缩量时追入 X"（放量突破时自动解除，缩量时仍有效）
    - 3 条覆盖：止损纪律 > 入场确认条件 > 仓位控制
...
- bias_checks 恰好 3 条，条件式纪律命令，不做方向预测，只约束操作纪律。"""
