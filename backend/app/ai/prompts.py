...
- bias_checks: array，**恰好 3 条**当前禁止/必须执行的纪律命令，每条形如：
    {
      "bias": "anchoring" | "endowment" | "disposition" | "confirmation"
             | "recency" | "availability" | "loss_aversion" | "overconfidence"
             | "herding" | "sunk_cost",
      "label": "3-6 字动词短语标签，如 '破位必走'、'禁止追高'、'设好止损'",
      "command": "一句话命令，<=25 字。要么是禁止句（'禁止 XXX'），
                  要么是必须句（'必须 XXX'）。含具体价位。
                  例如：'禁止在 0.80 破位后继续持有'
                  例如：'必须在买入时挂好 15.20 止损单'"
    }
  **风格约束**：
    - 每条只有一句话，不解释原因，不用"不要…因为…"格式
    - "禁止"开头 = 不能做的事；"必须"开头 = 一定要做的事
    - 必须含当前具体价位/技术位数字
...
- bias_checks 恰好 3 条，每条只有一句"禁止…"或"必须…"命令，含具体价位。"""
