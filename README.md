...
**选股打分**：综合分 = 0.70·金叉延续性 + 0.20·波段适配 + 0.10·股息。**日线/周线/日周综合** 三视图切换——weekly 看方向、daily 定时机，综合页 7 档 `strong_buy / buy / deep_pullback_entry / light_buy / watch_buy / watch / avoid`，详情页给「当前已涨 / 剩余中位预期 / 剩余平均预期」三维可预期涨幅。趋势状态机 8 态驱动决策（pullback_entry 可入手 / left_entry 左侧轻仓 …），过峰信号 0-100 置信度分级（按周期校准）。

**自选管理**：多对多分组 + 批量加入/移出 + 乐观刷新（操作与提示同步变）。全 A + ETF 元数据已内置 7569 条，添加股票联想秒出。
...
│   ├── seed.sqlite             # 打包内置：全 A 元数据种子库（CI 构建生成）
│   └── app/
...
├── frontend/src/
│   ├── api/                    # HTTP 层（一文件一领域）
│   ├── pages/                  # StockList 默认页 / StockDetail / Scoreboard / Positions / Compare / SyncLogs
